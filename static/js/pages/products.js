...
export const productMethods = {
  ...
  // Tab switcher for products/categories merged page
  ...
  async openProductModal(id) {
    ...
    if (id) {
      ...
      if (!p) {
        try { p = await API.get(`/api/products/${id}`); } catch {}
      }
      ...
      if (catObj && catObj.parent_id) {
        cat.value = catObj.parent_id;
        this.updateProductSubCategories();
        subCat.value = p.category_id;
      }
  ...
  async saveProduct() {
    ...
    const data = {
      ...
      name,
      category_id: subCat || mainCat || null,
    ...
    try {
      ...
      this.toast(id ? 'تم تعديل المنتج' : 'تم إضافة المنتج');
  ...
  async importExcel(input) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error(err.detail || 'فشل');
      }
      ...
      if (r.errors && r.errors.length) {
        ...
        this.toast(`تم استيراد ${r.added} منتج (تم تخطي ${r.skipped_duplicates || 0} مكرر)`);
      }
      await this.loadProductsPage();
    } catch (e) { this.toast(e.message, 'error'); }
    finally { input.value = ''; }
  ...
  bindDownloadLinks() {
    // Intercept protected downloads so HttpOnly cookie credentials are included.
    document.querySelectorAll('a[href*="/export"], a[href*="/pdf"]').forEach(a => {
      ...
      a.addEventListener('click', async (ev) => {
        ...
        try {
          ...
          const filename = m ? m[1] : (a.href.split('/').pop() || 'download');
